...
if False:
    ...
    telL = []
    for f in fL:
        telL.append(pd.read_csv(baseDir + "rem/raw/telemetry/" + f))
    ...
    control = ['latency_camera','joystick_hz','joystick_latency','joystick_initialized','e2e_latency']
    board = ['modem_report','wheel_speed','control_room_ram_usage','vehicle_ram_usage','control_room_cpu_usage','vehicle_cpu_usage','vehicle_disk_available','vehicle_internal_drive_state','i_frame_sent']
    asset = ['control_room_jitter','vehicle_jitter','control_room_ping','vehicle_ping','fault_detected','rtp_stats','jitter','nav_fix','nav_fix_filtered','received_control_message_stats', 'radar_track','vehicle_physics','steering_wheel_state','steering_angle','brake_pressure','throttle_pressure','bitrate_adjusted','vehicle_temperature_readings','control_room_performance','steering_interval_max','health_status']
    ...
    plt.hist(tel['delta_stream'],bins=30,label="delay streaming",alpha=0.3)
    plt.hist(tel['delta_proc'],bins=30,label="delay processing",alpha=0.3)
    plt.legend()
